...
const AnyReactComponent = ({ text, classname}) => (
  ...
  <div className={'marker-info ' + classname}>{text}</div>
);
class Home extends React.Component<{}> {
  ...
  showMapDiv = event => {
    ...
    //   var orderlocation = firebase
    //     .database()
    //     .ref('order_location/' + event.key + '/');
                    ...
                    <TableCell colSpan={7} className="map-content">
                      <div className="map-details">
                        <div className="delivery-type">
                          Delievery Type: &nbsp; &nbsp;<b>Eco</b> &nbsp; &nbsp;{' '}
                          <sup>$</sup>
                          <b className="amount">{n.price}</b>{' '}
                          <span className="refund-text">Refund</span>
                        </div>
                      ...
                      <div className="google-map">
                        <GoogleMapReact
                          bootstrapURLKeys={{
                            key: 'AIzaSyDXC-VDfNvIbIdRmvznzQjKyzlJgIvlvo0',
                          }}
                          defaultCenter={reactThis.props.center}
                          center={{
                            lat: n.receiverLocation.Latitude,
                            lng: n.receiverLocation.Longitude,
                          }}
                        ...
                        > 
                         {n.receiverLocation ? 
                           <AnyReactComponent
                            ...
                            text={'Reciver Point'}
                            classname={'green'} 
                            onClick={reactThis.popup}
                          />
                          : ''}
                             
                          {  n.senderLocation ? 
                            <AnyReactComponent
                            lat={n.senderLocation.Latitude}
                            lng={n.senderLocation.Longitude}
                            text={'Sender Location'}
                            classname={'blue'}
                          />  :''}
                          {n.pickupLocation ? 
                          <AnyReactComponent
                            lat={n.pickupLocation.Latitude}
                            lng={n.pickupLocation.Longitude}
